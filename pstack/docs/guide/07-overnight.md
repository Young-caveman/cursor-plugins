...
Before the skill hands back its summary, it delegates a read-only review to a pool entry from a different provider than the one that did the work; if the pool has none, it says so. The reply ends with an Attention section listing what deserves your scrutiny. Read that section first, then the log rows it points at. You're auditing decisions, not re-reading the whole night.
