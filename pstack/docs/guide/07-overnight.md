...
Before the skill hands back its summary, it spawns a reviewer using an allowed `judgment and prose` profile, preferring a different model family from the worker when configured. The reply ends with an Attention section listing what deserves your scrutiny. Read that section first, then the log rows it points at. You're auditing decisions, not re-reading the whole night.
