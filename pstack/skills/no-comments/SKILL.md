---
name: no-comments
description: "Run only when the user asks for this skill by name or a workflow the user started routes here. Spawn Comment Sicko, fix accepted findings, and offer encodings for claimed constraints."
...
1. Delegate one child per [T3 delegation](../setup-pstack/references/t3-delegation.md) on a `default`-tier pool entry. The brief is [`references/comment-sicko.md`](references/comment-sicko.md) verbatim, then the scope. It edits comments, so it is the only writer in this checkout while it runs. Do not restate its rules.
