...
description: "Run only when the user asks for this skill by name or a workflow the user started routes here. Generate a project-local verification skill that drives your app the way a user does — any language, framework, or platform. Use for /create-verification-skill, \"make a control skill for this repo\", or when a project has no scripted way to prove UI/CLI/service behavior."
disable-model-invocation: true
---

# Create a verification skill

Every serious project needs a scripted way to drive the real app and prove behavior: launch it, exercise a feature the way a user would, and capture evidence. This skill generates that as a project-local skill (`.agents/skills/verify-<app>/`, linked into `.claude/skills/` for Claude Code) tailored to the repo. You write the generator's output for the next agent, not for a human: it will be read cold, mid-task, by an agent that has never seen the app.
...
- **Machine:** which OS can run and drive each surface, and is it this one? Agents here cannot reach a different computer (see [T3 delegation](../setup-pstack/references/t3-delegation.md#one-machine-only)). For a surface only another machine can drive, generate its skill anyway, but open its Doctor with a platform check that reports `verified-unreachable: needs <platform>` and stops, and mark the surface in the feature map. Don't invent a proof for it, and don't count it as done in step 4.
...
Write `.agents/skills/verify-<app>/SKILL.md` with YAML frontmatter (`name: verify-<app>`, a `description` that names the app, the surface, and when to reach for it — without frontmatter the skill never registers — and `metadata: {pstack-generated-by: create-verification-skill@1}`). Then add the Claude Code link from the project root: `mkdir -p .claude/skills && ln -s ../../.agents/skills/verify-<app> .claude/skills/verify-<app>`. Commit both; the skill belongs to the project. Give it these sections, each grounded in what the interview actually found (no placeholders left):
