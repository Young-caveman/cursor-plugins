...
**Preserve the workflow's agent instructions when delegating.** Cursor playbook delegates use `subagent_type: "poteto-agent"`; routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) may prescribe another type. On Codex or OpenCode, use that equivalent when available. Do not replace a required agent type solely to obtain a model profile; pass the resolved model at invocation when the tool supports it.

**Model routing for every delegated call.** Resolve the task's named role through `~/.config/pstack/models.json` using the [PStack model routing contract](../setup-pstack/references/model-routing.md). The selected profile supplies the model and reasoning option for the active harness. A skill may choose a different profile only from that role's `allowed` list; state the reason and never silently raise effort or cost. Cross-cutting designs, gnarly concurrency, and subtle algorithms use the `hardest tasks` role; other implementation delegates use their playbook role. Mechanical edits may choose a faster profile only when it is allowed for that role. If setup has not produced a valid profile for the role and harness, ask for setup or an explicit model choice before delegating. Do not use the old Cursor slug defaults as Codex/OpenCode models. Keep background, agent-mode, and context-passing decisions separate from model routing.

You own every subagent's work. Review the diff and write your own summary, don't pass through what it said. Interrupt-chained resumes silently drop directives, so fire a fresh subagent with consolidated scope rather than trusting a "done" summary. For a second opinion, choose an allowed profile from the relevant role and prefer a different model family when configured. Agreement across distinct model families is stronger evidence than repeated calls to one model.
