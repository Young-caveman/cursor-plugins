...
- **Worker / verifier.** Use the unit's configured role and only profiles allowed for that role in the [PStack routing policy](../../setup-pstack/references/model-routing.md). Always `environment: "cloud"` unless the task needs this machine: `control-ui` or `control-cli` runtime verification (from `cursor-team-kit`), local transcripts under `agent-transcripts/`, simulators and local IDE state, or auth that exists only here. Cloud agents cannot read the local store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family when an allowed profile supports it; otherwise report that the configured choices do not provide one.
...
- Retry by mode: cap-hit or oom, respawn with smaller scope. Network-drop, retry as-is. Tool-error, retry once using another profile allowed for that unit's role; if none is allowed, report the constraint and use the existing profile only when retrying the same request is safe. Unknown, retry once. Two retries, then abandon the unit and replan around it.
