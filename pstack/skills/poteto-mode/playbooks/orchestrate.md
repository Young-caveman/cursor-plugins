...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, messaged, and drained only through T3's orchestration tools (`delegate_task`, `t3_thread_launch`, `t3_thread_send`, `task_status`, `t3_thread_read`), per [T3 delegation](../../setup-pstack/references/t3-delegation.md). State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nested children use the same T3 tools). Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** `default`-tier pool entries per [T3 delegation](../../setup-pstack/references/t3-delegation.md); `escalation` only for a unit that is genuinely hard or already failed. A writer gets its own worktree through `t3_thread_launch`; a read-only verifier can be a `delegate_task` child. Workers see only their brief, so it inlines what they need or points at repo paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different provider from its writer when the pool has one; otherwise report that it does not. Live verification goes through the project's verification skill (`verify-<app>`).
...
- Exactly one stacker per stack may run `gt`, serialized within its stack. Record the holder in the standing orders. Restacks run in the stacker's own worktree thread, never in the coordinator's checkout.
...
- After a T3 or harness restart: T3 threads and delegated tasks persist, but in-flight turns may have stopped. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach work with `t3_thread_list` and by PR and branch, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
