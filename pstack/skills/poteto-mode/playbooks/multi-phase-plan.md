...
Each live lane runs in its own worktree at the PR head (`t3_thread_launch`). Drive through the project's verification skill (`verify-<app>`, made by `/create-verification-skill`).
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on `default`-tier pool entries at the PR head, per the boot recipe.
