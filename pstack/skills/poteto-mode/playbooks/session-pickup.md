...
1. Locate the prior trail. A prior T3 thread or harness session in this project (per [reading past conversations](../../setup-pstack/references/transcripts.md); never another project's), or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
