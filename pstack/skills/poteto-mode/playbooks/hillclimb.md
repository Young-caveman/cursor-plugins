...
5. Loop, one hypothesis per iteration:
   - Hand the change to one child on a `default`-tier pool entry per [T3 delegation](../../setup-pstack/references/t3-delegation.md) (an `escalation` entry only when the change is genuinely hard) with a tight scope. Supervise and review the diff rather than typing it (the **guard-the-context-window** principle skill). When several independent hypotheses are live, fan them to parallel `t3_thread_launch` threads, each in its own worktree (the **separate-before-serializing-shared-state** principle skill).
