...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to one child on a `default`-tier pool entry per [T3 delegation](../../setup-pstack/references/t3-delegation.md) (an `escalation` entry only when the change is genuinely hard). Review the diff. Capture a post-fix trace.
