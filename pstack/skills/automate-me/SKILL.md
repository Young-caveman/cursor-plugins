...
Locate this project's conversation history before fanning out, per [reading past conversations](../setup-pstack/references/transcripts.md). Never read another project's history.

Survey recent agent conversations within that scope for recurring patterns. Run multiple parallel read-only children per [T3 delegation](../setup-pstack/references/t3-delegation.md), on `default`-tier pool entries, across slices of history (e.g. last 2-4 weeks, split into 3 slices so each has enough material). Each slice-mining child reads the threads or session files the parent names, looks for the signals below, and returns a short structured list of patterns it saw with evidence pointers. Default signals worth hunting:
...
Mining misses intent that hasn't come up yet. Use the harness's structured-question tool when it has one (Claude Code's `AskUserQuestion`, for example) rather than asking the user to type from scratch; otherwise ask in chat with numbered options.
