...
Find chat history per [reading past conversations](../setup-pstack/references/transcripts.md): T3 threads in this project first, harness logs as fallback.
...
3. Fan out across your chat history. Launch parallel read-only children per [T3 delegation](../setup-pstack/references/t3-delegation.md) on `default`-tier pool entries, each taking a slice of the corpus. Tell every child to order candidates by recency (thread `updatedAt`, or file modification time) and never by ID, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the thread or session ID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
